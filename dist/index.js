...
async function upsertUser(user) {
  ...
  try {
    ...
    await db.collection("users").updateOne(
      ...
      {
        $set: updateSet,
        $setOnInsert: { createdAt: /* @__PURE__ */ new Date(), updatedAt: /* @__PURE__ */ new Date() }
      },
      { upsert: true }
    );
    await db.collection("users").updateOne(
...
async function upsertDiscordUser(data) {
  const db = await getDb();
  if (!db) {
...
async function getDiscordUserByDiscordId(discordId) {
  const db = await getDb();
  if (!db) {
...
async function getVerifiedDiscordUsers() {
  const db = await getDb();
  if (!db) {
  ...
  try {
    const result = await db.collection("discordUsers").find({ status: "verified" }).sort({ verifiedAt: -1 }).toArray();
    return result;
  } catch (error) {
...
function generateAuthorizationUrl(redirectUri) {
  const clientId = process.env.DISCORD_CLIENT_ID;
  if (!clientId) {
...
var appRouter = router({
  ...
  discord: router({
     ...
     */
    getAuthUrl: publicProcedure.input(z2.object({ redirectUri: z2.string().url() })).query(({ input }) => {
     ...
     */
    verify: publicProcedure.input(z2.object({ code: z2.string(), redirectUri: z2.string().url() })).mutation(async ({ input }) => {
      try {
     ...
     */
    getStatus: publicProcedure.input(z2.object({ discordId: z2.string() })).query(async ({ input }) => {
